...
class Conv_GN_Relu(nn.Module):
    def __init__(self,inc,outc,k=3):
        super().__init__()
        self.op=nn.Sequential(
                nn.Conv2d(inc, outc, kernel_size=k, stride=1, padding=k//2),
...
class TDBCSNet_decoder(nn.Module):
    def __init__(self,
                 img_size=[480, 640],
                 in_channels=[96, 192, 384, 768],  # [64, 128, 320, 512],
                 num_classes=40,
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 0:
                ...
                # B, 60, 80, 192 -> B, 120, 160, 96
                # B, 120, 160, 96 -> B, 120, 160, 96
                layer_up = CGR_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                    input_resolution=(
                                        self.patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                        self.patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                                    depth=depths[(self.num_layers - 1 - i_layer)],
        ...
        self.convup1 = nn.Sequential(
            Conv_GN_Relu(inc=embed_dim // 2 * 3, outc=embed_dim // 2),
            Conv_GN_Relu(inc=embed_dim // 2, outc=embed_dim // 2),
        )
        self.convup0 = nn.Sequential(
            Conv_GN_Relu(inc=embed_dim // 4 * 3, outc=embed_dim // 4),
            Conv_GN_Relu(inc=embed_dim // 4, outc=embed_dim // 4),
        )
