...
class DSM(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.fc_layers(v).reshape([b,2,c//2]).softmax(dim=1).reshape([b,c]).view(b, c, 1, 1)
        v = self.sigmoid(v)
        return x * v
class TAM(nn.Module):
    def __init__(self,inc,outc,split_ratio=0.5,kernel_size=9,extend_scope=1.0,if_offset=True):
        super().__init__()
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.dsc_inc=int(inc*split_ratio)
        self.k_inc=inc-self.dsc_inc

        self.dsc_outc = int(outc * split_ratio)
        self.k_outc = outc - self.dsc_outc

        self.convs = EncoderConv(inc, outc)

        self.convx_dsc = DSConv(
            self.dsc_inc,
            self.dsc_outc,
            kernel_size,
            extend_scope,
        ...
        self.convx = nn.Sequential(
            ...
            nn.GroupNorm(self.k_outc//4,self.k_outc),
            nn.ReLU(),
        )



        self.convy_dsc = DSConv(
            self.dsc_inc,
            self.dsc_outc,
            kernel_size,
            extend_scope,
        ...
        self.se = DSM('avg', 2 * self.k_outc, 2)
        self.conv = EncoderConv(3 * outc, outc)


    def forward(self,x):
        xs = self.convs(x)

        x_dsc,x_k=torch.split(x,[self.dsc_inc,self.k_inc],dim=1)

        dscx_out = self.convx_dsc(x_dsc)
        dscy_out = self.convy_dsc(x_dsc)
        kx_out = self.convx(x_k)
        ky_out = self.convy(x_k)
        k_out=self.se(cat([kx_out,ky_out],dim=1))
        out = self.conv(cat([xs, dscx_out, dscy_out,k_out], dim=1))
        return out
class TopologyawareBranch(nn.Module):
    def __init__(self,layers,dim=16,inc=3,split_ratio=0.5):
        self.depth=layers
        self.dim=dim
        super().__init__()
        ...
        for i in range(self.depth)[2:]:
            if i==0:
                self.layers.append(TAM(inc=inc, outc=dim * (2 ** i),split_ratio=self.split_ratio))
            else:
                self.layers.append(TAM(inc=dim*(2**(i-1)),outc=dim*(2**i),split_ratio=self.split_ratio))
