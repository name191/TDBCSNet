...
class crossblock_for_ChannelAttentionBlock_cross(nn.Module):
    def __init__(self, inc, outc, l=9,ifse=True,norm="gn",group=4):
        super(crossblock_for_ChannelAttentionBlock_cross, self).__init__()
        self.ifse=ifse
        if norm=="gn":
    ...
    def forward(self, x):
        ...
        out = self.conv(out)
        return out
class ChannelAttentionBlock_cross(nn.Module):

    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30):
        super(ChannelAttentionBlock_cross, self).__init__()

        self.cab = nn.Sequential(
            crossblock_for_ChannelAttentionBlock_cross(inc=num_feat,outc=num_feat,norm='gn'),
...
class CMM(nn.Module):
    ...
    def __init__(
        ...
        self.drop_path = DropPath(drop_path)
        self.conv_blk = ChannelAttentionBlock_cross(hidden_dim)
        self.norm2 = norm_layer(hidden_dim)
...
class Context_branch(VSSM):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.layer0 = nn.Sequential(
            nn.Conv2d(in_chans, self.embed_dim // 4, kernel_size=3, stride=1, padding=1),
...
class Front_Net(nn.Module):
    def __init__(self,inc=3,resultdim=32,norm='bn'):
        super(Front_Net,self).__init__()
        self.numbers=resultdim//2
        if norm == "bn":
            self.layer1 = nn.Sequential(
                nn.Conv2d(inc, self.numbers, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(self.numbers),
                nn.ReLU(),
                nn.Conv2d(self.numbers, self.numbers, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(self.numbers),
                nn.ReLU()
            )
            self.layer2 = nn.Sequential(
                nn.Conv2d(self.numbers, self.numbers*2, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(self.numbers*2),
                nn.ReLU(),
                nn.Conv2d(self.numbers*2, self.numbers*2, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(self.numbers*2),
                nn.ReLU()
            )
        elif norm == "gn":
            self.layer1 = nn.Sequential(
                nn.Conv2d(inc, self.numbers, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(self.numbers//4,self.numbers),
                nn.ReLU(),
                nn.Conv2d(self.numbers, self.numbers, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(self.numbers//4,self.numbers),
                nn.ReLU()
            ...
            self.layer2 = nn.Sequential(
                nn.Conv2d(self.numbers, self.numbers * 2, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(self.numbers * 2//4,self.numbers * 2),
                nn.ReLU(),
                nn.Conv2d(self.numbers * 2, self.numbers * 2, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(self.numbers * 2//4,self.numbers * 2),
                nn.ReLU()
...
class LVSSM(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.use_checkpoint = use_checkpoint
        self.dim_mamba=int(hidden_dim*split_ratio)
        self.dim_res=hidden_dim-self.dim_mamba
        self.norm = norm_layer(self.dim_mamba)
        self.op = SS2D(
    ...
    def _forward(self, input: torch.Tensor):
        x_mamba,x_res=torch.split(input,[self.dim_mamba, self.dim_res],dim=-1)

        out_mamba = x_mamba + self.drop_path(self.op(self.norm(x_mamba)))
        if self.mlp_branch:
            out_mamba = out_mamba + self.drop_path(self.mlp(self.norm2(out_mamba)))  # FFN

        out=torch.cat([out_mamba,x_res],dim=-1)
        out=self.proj(out)
        return out
...
class VSSM_light(nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
        ...
        return super()._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
                                             error_msgs)
class SemanticBranch(VSSM_light):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
                 depths=[2, 2, 9, 2], dims=[96, 192, 384, 768],split_ratios = 0.5,
                 d_state=16, ssm_ratio=2.0, attn_drop_rate=0.,
                 drop_rate=0., drop_path_rate=0.1, mlp_ratio=4.0,
                 patch_norm=True, norm_layer=nn.LayerNorm,
                 downsample_version: str = "v2",
        ...
        super().__init__(patch_size=patch_size, in_chans=in_chans, num_classes=num_classes,
                         depths=depths, dims=dims, split_ratios =split_ratios,
                         d_state=d_state, ssm_ratio=ssm_ratio, attn_drop_rate=attn_drop_rate,
                         drop_rate=drop_rate, drop_path_rate=drop_path_rate, mlp_ratio=mlp_ratio,
                         patch_norm=patch_norm, norm_layer=norm_layer,
                         downsample_version=downsample_version,
                         ...
                         # print(depths)
        out_indices=tuple(range(len(depths)))
        self.front=Front_Net(resultdim=self.embed_dim // 2,norm='bn')
        self.layer2 = nn.Sequential(
            nn.Conv2d(self.embed_dim // 2, self.embed_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.embed_dim),
            Permute(0, 2, 3, 1),
